...
`prompt-guide init` copies this tree into **`.claude/`** at the repo root (`core/`, `context/`, …). That path is the default **`layers.source`**. Use **`--layer-target`** if you need another copy.

**Bundles:** `prompt.config.js` → `layers.source` (default `.claude`). `prompt-guide install` reads **only that path** when building Codex / Claude / Windsurf outputs and when generating the Cursor manifest.
